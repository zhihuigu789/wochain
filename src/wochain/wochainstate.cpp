...
#include "wochainstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

WochainState::WochainState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = WochainState::openDB(_path + "/wochainDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

WochainState::WochainState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute WochainState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, WochainTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createWochainAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            wochain::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> WochainState::vins() const // temp
{
...
void WochainState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* WochainState::vin(dev::Address const& _a) const
{
    return const_cast<WochainState*>(this)->vin(_a);
}

Vin* WochainState::vin(dev::Address const& _addr)
{
...
// void WochainState::commit(CommitBehaviour _commitBehaviour)
// {
...
void WochainState::kill(dev::Address _addr)
{
...
void WochainState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address WochainState::createWochainAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void WochainState::deleteAccounts(std::set<dev::Address>& addrs){
...
void WochainState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void WochainState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
