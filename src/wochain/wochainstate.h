...
#include <wochain/wochaintransaction.h>
...
class WochainState : public dev::eth::State {
    
public:

    WochainState();

    WochainState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, WochainTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
struct TemporaryState{
    std::unique_ptr<WochainState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<WochainState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(WochainState* _state, const std::vector<TransferInfo>& _transfers, const WochainTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const WochainTransaction& transaction;
