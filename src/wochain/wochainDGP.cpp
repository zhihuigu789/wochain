#include "wochainDGP.h"

void WochainDGP::initDataEIP158(){
...
bool WochainDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule WochainDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t WochainDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t WochainDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t WochainDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t WochainDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool WochainDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void WochainDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void WochainDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void WochainDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void WochainDGP::createParamsInstance(){
...
dev::Address WochainDGP::getAddressForBlock(unsigned int blockHeight){
...
void WochainDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void WochainDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void WochainDGP::parseStorageOneUint64(uint64_t& value){
...
void WochainDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule WochainDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
