...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setWochainSchedule(wochainDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = wochainDGP.getBlockSize(nHeight);
    minGasPrice = wochainDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = wochainDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    WochainTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractWochainTX resultConverter;
    if(!convert.extractionWochainTransactions(resultConverter)){
    ...
    for(WochainTransaction wochainTransaction : wochainTransactions){
        txGas += wochainTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + wochainTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(wochainTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, wochainTransactions, hardBlockGasLimit);
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("wochaincoin-miner");
...
void StakeWochains(bool fStake, CWallet *pwallet)
{
