...
bool WalletModel::isUnspentAddress(const std::string &wochainAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if(fValidAddress && CBitcoinAddress(address).ToString() == wochainAddress && out.tx->tx->vout[out.i].nValue)
        {
            return true;
        }
    }
