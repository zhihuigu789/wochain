    ...
    platformStyle(_platformStyle)
{
    ...
    wochainVersionChecker = new WochainVersionChecker(this);

    if(fCheckForUpdates && wochainVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(WOC_RELEASES, WOC_RELEASES);
        QString message(tr("New version of Wochain wallet is available on the Wochain source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Wochain address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and wochain: URIs)"));
...
//注释智能合约
  /**  QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/qrctoken"), tr("&QRC Tokens"), this);
    ...
    **/
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Wochain addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Wochain addresses"));
    ...
    openAction->setStatusTip(tr("Open a wochain: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Wochain command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Wochain network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Wochain"); // default title
