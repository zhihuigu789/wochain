...
bool ToHash160(const std::string& strWochainAddress, std::string& strHash160)
{
    CBitcoinAddress wochainAddress(strWochainAddress);
    if(wochainAddress.IsValid()){
        CKeyID keyid;
        wochainAddress.GetKeyID(keyid);
...
bool ToWochainAddress(const std::string& strHash160, std::string& strWochainAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress wochainAddress;
    wochainAddress.Set(keyid);
    if(wochainAddress.IsValid()){
        strWochainAddress = wochainAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToWochainAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToWochainAddress(tokenEvent.receiver, tokenEvent.receiver);
