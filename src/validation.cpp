...
#if defined(NDEBUG)
# error "Wochain cannot be compiled without assertions."
#endif
...
std::unique_ptr<WochainState> globalState;
...
const std::string strMessageMagic = "Wochain Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // wochain
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            WochainDGP wochainDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = wochainDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = wochainDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            WochainTxConverter converter(tx, NULL);
            ExtractWochainTX resultConverter;
            if(!converter.extractionWochainTransactions(resultConverter)){
            ...
            std::vector<WochainTransaction> wochainTransactions = resultConverter.first;
            std::vector<EthTransactionParams> wochainETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(WochainTransaction wochainTransaction : wochainTransactions){
                sumGas += wochainTransaction.gas() * wochainTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, wochainTransaction.gasPrice());
                } else {
                    txMinGasPrice = wochainTransaction.gasPrice();
                }
                VersionVM v = wochainTransaction.getVersion();
                if(v.format!=0)
                ...
                if(wochainTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(wochainTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(wochainTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += wochainTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)wochainTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(wochainETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > wochainTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s WOC additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // wochain
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // wochain
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    WochainTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<WochainTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path wochainDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(wochainDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(wochainDir.string(), std::ios::in | std::ios::out);
...
bool WochainTxConverter::extractionWochainTransactions(ExtractWochainTX& wochaintx){
    std::vector<WochainTransaction> resultTX;
    ...
    wochaintx = std::make_pair(resultTX, resultETP);
    return true;
}

bool WochainTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool WochainTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
WochainTransaction WochainTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    WochainTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = WochainTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = WochainTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // wochain
    WochainDGP wochainDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setWochainSchedule(wochainDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = wochainDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = wochainDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = wochainDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // wochain
...
///////////////////////////////////////////////////////////////////////////////////////// wochain
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            WochainTxConverter convert(tx, &view, &block.vtx);

            ExtractWochainTX resultConvertWochainTX;
            if(!convert.extractionWochainTransactions(resultConvertWochainTX)){
            ...
            if(!CheckMinGasPrice(resultConvertWochainTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertWochainTX.first, blockGasLimit);
            ...
            for(WochainTransaction& qtx : resultConvertWochainTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertWochainTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertWochainTX.first[k].from(), resultConvertWochainTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(10000000000 + ((pindex->nHeight - 5000) * 380)) * COIN){
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // wochain
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // wochain
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // wochain
            globalState->setRootUTXO(oldHashUTXORoot); // wochain
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // wochain
        uint32_t sizeBlockDGP = wochainDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Wochain PoS we need to sync this after every block to ensure txdb is populated for
