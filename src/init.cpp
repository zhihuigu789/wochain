...
void Shutdown()
{
    ...
    if (pwalletMain)
    {
        StakeWochains(false, pwalletMain);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/wochainproject/wochain>";
    const std::string URL_WEBSITE = "<https://wochain.org>";
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateWochain";
                ...
                boost::filesystem::path wochainStateDir = GetDataDir() / "stateWochain";

                bool fStatus = boost::filesystem::exists(wochainStateDir);
                const std::string dirWochain(wochainStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsWochainstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<WochainState>(new WochainState(dev::u256(0), WochainState::openDB(dirWochain, hashDB, dev::WithExisting::Trust), dirWochain, existsWochainstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::wochainMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(wochainStateDir.string());
    ...
    if (!GetBoolArg("-staking", DEFAULT_STAKE))
        ...
        StakeWochains(true, pwalletMain);
#endif
    // ********************************************************* Step 12: finished
