...
void checkResult(bool isCreation, std::vector<WochainTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    WochainTxConverter converter(transaction, NULL);
    ExtractWochainTX wochainTx;
    BOOST_CHECK(converter.extractionWochainTransactions(wochainTx));
    std::vector<WochainTransaction> result = wochainTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(wochaintxconverter_tests, TestingSetup)
