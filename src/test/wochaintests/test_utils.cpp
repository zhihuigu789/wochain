...
void initState(){
    ...
    const std::string dirWochain = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<WochainState>(new WochainState(dev::u256(0), WochainState::openDB(dirWochain, hashDB, dev::WithExisting::Trust), dirWochain + "/wochainDB", dev::eth::BaseState::Empty));
...
WochainTransaction createWochainTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    WochainTransaction txEth;
    if(recipient == dev::Address()){
        txEth = WochainTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = WochainTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
