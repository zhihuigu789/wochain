...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//toniwochain edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // wochain
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // wochain
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // wochain
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // wochain
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress wochainSenderAddress(request.params[2].get_str());
        if(wochainSenderAddress.IsValid()){
            CKeyID keyid;
            wochainSenderAddress.GetKeyID(keyid);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of wochain addresses\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // wochain
