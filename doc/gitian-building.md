...
*Setup instructions for a Gitian build of Wochain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Wochain
...
- [Building Wochain Core](#building-wochain-core)
...
Clone the git repositories for wochain and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/wochainproject/wochain --recursive
...
```bash
sudo cp /home/debian/wochain/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the wochain repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Wochain Core
----------------

To build Wochain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the wochain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/wochain/.git/
...
For example:
```bash
URL=https://github.com/laanwj/wochain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit wochain=${COMMIT} --url wochain=${URL} ../wochain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit wochain=${COMMIT} --url wochain=${URL} ../wochain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit wochain=${COMMIT} --url wochain=${URL} ../wochain/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-wochain:
./bin/gbuild --commit wochain=${COMMIT},cpp-eth-wochain=${ETHCOMMIT} --url wochain=${URL},cpp-eth-wochain=${ETHURL} ../wochain/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the wochain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../wochain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/wochain
SIGPATH=/some/root/path/wochain-detached-sigs

./bin/gbuild --url wochain=${BTCPATH},signature=${SIGPATH} ../wochain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/wochain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/wochain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/wochain-osx-build.assert
```
...
[wochainproject/gitian.sigs](https://github.com/wochainproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@wochain.org) and he will commit them.
